...
struct array* dijkstra(long source, long target, uint64_t amount, struct network* network, uint64_t current_time, long p, enum pathfind_error *error) {
  ...
  distance[p][target].hop_number = 0;
  ...
  while(heap_len(distance_heap[p])!=0) {
    ...
    to_node_dist = distance[p][best_node_id];     // struct hai
    amt_to_send = to_node_dist.amt_to_receive;    // 

    best_node = array_get(network->nodes, best_node_id);    // to get edges from the best_node_id
    /* best_edges = get_best_edges(best_node_id, amt_to_send, source, network); */

    for(j=0; j<array_len(best_node->open_edges); j++) {
      edge = array_get(best_node->open_edges, j);     // C -> B wali edge
      edge = array_get(network->edges, edge->counter_edge_id);    // B -> c wali edge update ho gyi h

      from_node_id = edge->from_node_id;    // B
      if(from_node_id == source){
        if(edge->balance < amt_to_send)
          continue;
      }
      else{
        channel = array_get(network->channels, edge->channel_id); // B-C biderectional edge
      ...
      amt_to_receive = amt_to_send + edge_fee;      // Amount B will send to C, will send more as some money will be subtracted in this process
      ...
      edge_weight = get_edge_weight(amt_to_receive, edge_fee, edge_timelock); // Time Fee lock ho skta h
      tmp_weight = to_node_dist.weight + edge_weight;
      tmp_weight = tmp_weight + to_node_dist.hop_number + 1;
      ...
      current_dist = distance[p][from_node_id].distance;   // currently B
      ...
      distance[p][from_node_id].hop_number = 1 + to_node_dist.hop_number;
